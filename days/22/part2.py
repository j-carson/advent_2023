...
def part_2(bricks: list[Brick]) -> int:
    score = 0

    shape = (
        max(b.max("x") for b in bricks) + 1,
        max(b.max("y") for b in bricks) + 1,
        max(b.max("z") for b in bricks) + 1,
    )

    world = np.full(shape, -1, dtype=np.int16)
    for i, brick in enumerate(bricks):
        for xyz in brick.range("xyz"):
            world[xyz] = i

    supports = defaultdict(list)
    for level in range(1, shape[-1] - 1):
        bricks_on_level = np.unique([b for b in world[:, :, level]])
        map_above = world[:, :, level + 1].squeeze()

        for i in bricks_on_level:
            if i == -1:
                continue
            brick = bricks[i]
            above = np.unique([map_above[xy] for xy in brick.range("xy")])
            supports[i] = [a for a in above if a not in (-1, i)]
    ...
    for i, brick in enumerate(bricks):
        ...
        # get a count of how many supports each brick has
        number_below = defaultdict(int)
        for supporting in supports.values():
            for s in supporting:
                number_below[s] += 1

        # pick the starting brick
        chain_react = [i]

        # work all chain reactions
        while len(chain_react):
            j = chain_react.pop()
            if bricks[j].marked:
                continue
            bricks[j].marked = True
            for s in supports[j]:
                number_below[s] -= 1
                if number_below[s] == 0:
                    chain_react.append(s)

        # tally up the damage
        tally = sum(b.marked for b in bricks) - 1
        score += tally
