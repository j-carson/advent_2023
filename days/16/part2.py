...
class MirrorPuzzle:
    ...
    def solve(self):
        ...
        for row in range(n_row):
            answer = self.solve_part(Point(row, -1), "east")
            if answer > result:
                result = answer

            answer = self.solve_part(Point(row, n_col), "west")
            if answer > result:
                result = answer

        for col in range(n_col):
            answer = self.solve_part(Point(-1, col), "south")
            if answer > result:
                result = answer

            answer = self.solve_part(Point(n_row, col), "north")
            if answer > result:
                result = answer
        return result

    def solve_part(self, starting_point, starting_dir):
        # reset
        self.visited = set()
        self.energized[:, :] = False
        assert len(self.workq) == 0

        # solve as in part1
        self.workq.append((starting_point, starting_dir))
